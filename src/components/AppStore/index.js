...
const AppStore = () => {
  ...
  //const [appsListArray, setAppsListArray] = useState(appsList);
  // if we have delete functionality to be performed on list, we have to maintain it in state otherwise not necessary

  const ActiveTabButton = (tabId) => {
  ...
  const getSearchResults = () => {
    const searchResults = appsList.filter((eachItem) => {
      return eachItem.appName.toLowerCase().includes(inputValue.toLowerCase());
    });
  ...
  // wrong way to filter Array you cannot update state inside function which causes re-renders and fails
  // const getFilteredArray = () => {
  //   const array = appsList.filter((eachItem) => {
  //     return eachItem.category === activeTabId;
  //   });
  //   //console.log(array);
  //   setAppsListArray(array);
  // };
  // getFilteredArray();

  // this is general way to filter array but in real word projects we have to use functions
  // const filteredArray = appsList.filter((eachApp) => {
  //   return eachApp.category === activeTabId;
  // });
  //console.log(filteredArray);

  const getFilteredArray = () => {
    const newArray = searchResults.filter((eachApp) => {
      return eachApp.category === activeTabId;
    });
    return newArray;
  };
        ...
        <div className="app-store">
          ...
          <div className="input-and-search">
            <input
              ...
              value={inputValue}
            />
            <button type="search" className="buttton-search-icon">
          ...
          <ul className="app-item-container">
            {filteredArray.map((eachAppItem) => (
              <AppItem appItemDetails={eachAppItem} key={eachAppItem.appId} />
            ))}
          </ul>
        </div>
      </div>
